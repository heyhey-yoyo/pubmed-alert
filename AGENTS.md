...
面向单用户的 PubMed 关键词邮件提醒工具。用户通过网页保存检索式和收件邮箱，Cloudflare Cron 每小时触发检查；发现新 PMID 时经 Resend 发送汇总邮件。
...
本地开发：先 `cp .dev.vars.example .dev.vars` 填真实密钥；`npm run dev` 后手动触发检查可用：
...
正文容器最大 720px、登录卡片最大 460px，品牌页眉内容区最大 1280px。存储不可用时允许登录与操作，并准确说明口令无法持久保存或清除；验证登录状态时使用隔离 API，不发送测试邮件。

### 界面维护约定

管理页面使用 `ydchen-portfolio` 的米白 / 赤陶色视觉系统；管理界面位于 `src/ui.ts`；修改展示层时须保持认证、检索窗口、Cron、邮件幂等和 Durable Object 状态语义。

视觉验收需保持说明、记住状态和页脚文字不小于 13px，并在 1440px 桌面与 390px 手机视口检查整体横向溢出。
